...
class Conformer(nn.Module):
	...
	def __init__(self, patch_size=16, in_chans=3, num_classes=1000, base_channel=64, channel_ratio=4, num_med_block=0,
		...
		self.trans_cls_head_fc = nn.Sequential(nn.Dropout(0.8), nn.Linear(embed_dim, num_classes))
		self.pooling = nn.AdaptiveAvgPool2d(1)
		self.conv_cls_head_fc = nn.Sequential(nn.Dropout(0.8), nn.Linear(int(256 * channel_ratio), num_classes))
...
def conform(numclass):
	modle = Conformer(patch_size=16, channel_ratio=4, embed_dim=384, depth=12,
	                  num_heads=6, mlp_ratio=4, qkv_bias=True, num_classes=numclass)
	modle.load_state_dict(torch.load('./model/Conformer_small_patch16.pth'), strict=False)
	return modle