...
__all__ = ['optical_trans']

import torch


class OpticalTrans(object):
...
class Scattering2D(object):
	...
	def build(self):
		...
		self.pad = Pad(
				[(self.M_padded - self.M) // 2, (self.M_padded - self.M + 1) // 2, (self.N_padded - self.N) // 2, (self.N_padded - self.N + 1) // 2],
				[self.M, self.N], pre_pad=self.pre_pad)
